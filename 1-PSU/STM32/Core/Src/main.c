...
/* USER CODE BEGIN PV */
uint16_t gBuffer_SPI[BUFSIZE_SPI];
uint16_t gBuffer_SPI2[BUFSIZE_SPI];
int g_SpiRxCnt=0;
uint8_t gSPIRxDone=0;
/* USER CODE END PV */
...
int main(void)
{
  ...
  while (1)
  {
	  ...
	  HAL_SPI_TransmitReceive_IT(&hspi2,(uint8_t*)&SDI_Data1,(uint8_t*)gBuffer_SPI,1);
	  while(!gSPIRxDone)
	  {
