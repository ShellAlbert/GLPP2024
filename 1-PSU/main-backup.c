...
//I tend to use TIM6 to generate interrupt by nS.
//But the minimum period is 10uS, it seems like something going wrong.
//I can't solve this problem, so I consider another approach __NOP().
#if 0
...
//Using an oscilloscope to test how much time __NOP() takes.
//We get a consequence of 2.2uS for a single __NOP() instrument code.
#if 0
	while(1){
...
//				strlen((char*) buffer_format), 0xFFFF);
		...
		//Transmit 16-bits ADC Data - HIGH Byte.
		hlpuart1.Instance->TDR=(gSPI_Buffer1[gSPI_RdPtr1]>>8);
		//Waiting for Transmit Data Register Empty.
		while(!(hlpuart1.Instance->ISR & USART_ISR_TXE))
		{}

		//Transmit 16-bits ADC Data - LOW Byte.
		hlpuart1.Instance->TDR=(gSPI_Buffer1[gSPI_RdPtr1]&0x00FF);
		//Waiting for Transmit Data Register Empty.
		while(!(hlpuart1.Instance->ISR & USART_ISR_TXE))
		{}
